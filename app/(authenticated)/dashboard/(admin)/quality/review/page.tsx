...
const AdminReviewPage = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = reviewRepository.hooks.useGetTranslatorReviews(10, page);

  if (error) return <div>Error loading data: {error.message}</div>;

  const filteredData =
    reviews?.data?.filter((review: Review) => {
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className='flex items-center gap-2'>
          ...
          <p className='font-semibold text-xs 2xl:text-sm line-clamp-1'>
            {record.translator.user.userDetail.fullName}
          </p>
        </div>
      ),
    },
  ...
  const data =
    filteredData.map((review: Review) => ({
      key: review.id,
      ...review,
    })) || [];
